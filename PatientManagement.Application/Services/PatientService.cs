    ...
    public class PatientService : IPatientService
    {
        ...
        public async Task<ExecutionResult<IEnumerable<PatientResponse>>> GetAllPatientsAsync()
        {
            try
            {
                ...
                _logger.LogInformation($"Retrieved {patients.Count()} active patients.");
                return ExecutionResult<IEnumerable<PatientResponse>>.Success(_mapper.Map<IEnumerable<PatientResponse>>(patients), "Active patients retrieved successfully.");
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error retrieving all active patients: {ex.Message}");
                return ExecutionResult<IEnumerable<PatientResponse>>.Failed("An error occurred while retrieving all active patients.");
            }
        }
        ...
        // Soft Delete Patient - Returns ExecutionResult<bool>
